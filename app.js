...
/* deer_mid is deliberately absent: the park already carries its own label. */
const LABELLED = ['hkv', 'sda_mkt', 'gp_mkt', 'gp_metro', 'aur_m'];
...
function drawMap() {
  ...
  svg.appendChild(el('path', {
    ...
    fill: 'rgba(255,255,255,.022)', stroke: 'rgba(255,255,255,.05)', 'stroke-width': 1,
  }));
  svg.appendChild(el('text', {
    x: dp[1].x - 26, y: dp[1].y - 4, fill: '#3A4150', 'font-size': 15,
  ...
  for (const e of EDGES) {
    ...
    svg.appendChild(el('line', {
      ...
      stroke: col, 'stroke-width': e.cls === 'trunk' ? 6 : e.cls === 'path' ? 3 : 4.5,
      'stroke-linecap': 'round', opacity: isOn ? 0.95 : 0.4,
      'stroke-dasharray': e.cls === 'path' ? '9 8' : '',
    }));
    ...
    for (let i = 0; i < n; i++) {
      const t = (i + 1) / (n + 1);
      svg.appendChild(el('circle', {
        cx: a.x + (b.x - a.x) * t + 9, cy: a.y + (b.y - a.y) * t - 9, r: 2.6,
  ...
  if (route) {
    ...
    svg.appendChild(el('polyline', {
      points: pts, fill: 'none', stroke: '#fff', 'stroke-width': 2.4,
  ...
  for (const h of HELP_POINTS) {
    ...
    svg.appendChild(el('rect', {
      ...
      fill: 'none', stroke: C_VERIFIED, 'stroke-width': 1.6, opacity: .7,
  ...
  for (const id of LABELLED) {
    const p = pNode(id);
    svg.appendChild(el('text', {
      x: p.x, y: p.y - 15, fill: '#6A7183', 'font-size': 13, 'text-anchor': 'middle',
    }, NODES[id].name.replace(' — ', ' ').replace('Green Park Metro Station', 'Green Park Metro')));
  }
  for (const id of [S.origin, JOURNEY.to]) {
    const p = pNode(id);
    svg.appendChild(el('circle', { cx: p.x, cy: p.y, r: 8, fill: 'none', stroke: '#fff', 'stroke-width': 2 }));
    svg.appendChild(el('circle', { cx: p.x, cy: p.y, r: 3.2, fill: '#fff' }));
  }
  ...
  if (route && S.progress > 0) {
    ...
    svg.appendChild(el('circle', { cx: p.x, cy: p.y, r: 15, fill: 'rgba(255,255,255,.12)' }));
    svg.appendChild(el('circle', { cx: p.x, cy: p.y, r: 6.5, fill: '#fff' }));
  }
}
...
function renderCheckin() {
  ...
  if (S.rerouted) {
    slot.innerHTML = `<div class="checkin done">
  ...
  if (!r || S.shadow || S.checkinAsked || S.progress < 35 || S.progress > 85) { slot.innerHTML = ''; return; }
  ...
  slot.innerHTML = `<div class="checkin">
...
function renderRoutes() {
  ...
  S.routes.forEach((r, i) => {
    ...
    d.className = 'route' + (i === S.selected ? ' sel' : '');
    d.onclick = () => { S.selected = i; paint(); };
...
function tick() {
  ...
  if (S.minute === 0) S.hour = (S.hour + 1) % 24;
