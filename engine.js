...
/* How much daylight there is, 0 to 1, with dawn and dusk ramps.
   Street lamps are irrelevant at 3pm — the sun is the lighting — so the
   Lit factor has to rise with daylight or the app would recommend the
   lamp-lined arterial over a park at noon for no reason at all. */
function daylight(hour) {
  if (hour >= 7 && hour < 17) return 1;
  if (hour >= 17 && hour < 19) return 1 - (hour - 17) / 2;   // dusk
  if (hour >= 5 && hour < 7) return (hour - 5) / 2;          // dawn
  return 0;
}

/* How much public transport is actually running. Delhi's last Metro is
   around 23:00 and bus headways stretch badly after 21:00, so a bus stop
   at 2am is not the escape route a bus stop at 6pm is. */
function serviceLevel(hour) {
  if (hour >= 6 && hour < 21) return 1;
  if (hour >= 21 && hour < 23) return 1 - (hour - 21) * 0.25;  // 1.0 -> 0.5
  if (hour >= 5 && hour < 6) return 0.6;
  return 0.25;                                                  // 23:00 - 05:00
}

function scoreSegment(edge, hour) {
  const isNight = hour >= 20 || hour < 6;

  // LIT — mapped street lamps per metre, plus whatever daylight there is.
  const lamps = clamp01(edge.lamps / (edge.lengthM / LAMP_SPACING_IDEAL));
  const dl = daylight(hour);
  const lit = clamp01(lamps + dl * (1 - lamps));
  ...
  // CONNECTED — can she leave this segment by public transport, right now?
  const reach = edge.transit
    ? clamp01(0.5 + 0.5 * Math.min(edge.busRoutes.length / 3, 1))
    : 0.15;
  const connected = reach * serviceLevel(hour);
